...
def test_admin_responses_columns_api_returns_77_question_columns(
    client: TestClient,
    db_session: Session,
    event_factory,
) -> None:
    ...
    data = response.json()
    assert len(data["questionColumns"]) == 77
    assert data["questionColumns"][0]["key"] == "q001"
    assert data["questionColumns"][-1]["key"] == "q077"
